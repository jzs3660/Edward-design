...
.hero-copy{width:800px}.hero h1 em{display:inline-block;white-space:nowrap}
</style></head><body>
...
const targets=[
  ['#hero','aident-ppt-hero.webp'],
  ['#system','aident-ppt-system.webp'],
  ['#showcase-en','aident-ppt-showcase.webp'],
  ['#showcase-zh','aident-ppt-showcase.zh.webp']
];
for(const [selector,file] of targets){
  const finalPath=path.join(previewDir,file);
  if(selector==='#system'){
    const size=await renderSystemPreview({canvasModule,sharp,root,tokens,components,fontManifest,iconManifest,backgroundManifest,outPath:finalPath});
  ...
  if(selector==='#showcase-en'||selector==='#showcase-zh'){
    ...
    const size=await renderShowcasePreview({canvasModule,sharp,root,language,slidePaths:slides.map(src=>path.resolve(outputDir,src)),outPath:finalPath});
    console.log(`${file}: ${size.width}×${size.height} · direct bundled-font canvas renderer`);
    continue;
  }
  ...
  const png=await page.screenshot({type:'png',clip:{x:box.x,y:box.y,width:box.width,height:box.height}});
  await sharp(png).webp({lossless:true,effort:6}).toFile(finalPath);
